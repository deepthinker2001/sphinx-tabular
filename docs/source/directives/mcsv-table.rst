...
Use ``mcsv-table`` for CSV-formatted tables whose ordinary cell values are
parsed as MyST Markdown. The directive uses the same merge, formula,
sticky-header, sorting, initial-sorting, and searching pipeline as
``rcsv-table`` and ``rlist-table``.
...
.. code-block:: rst
   ...
   .. mcsv-table:: External table
      :file: tables/versions.mcsv
      :header-rows: 1
...
Ordinary cell values are parsed with MyST Markdown. They may contain Markdown
emphasis, strong text, inline code, links, and MyST roles or other syntax
supported by the project's MyST configuration.

.. code-block:: rst

   .. mcsv-table:: Markdown cells
      :header-rows: 1
...
Because CSV uses commas as separators, quote a cell when its Markdown content
contains a comma:
...
``mcsv-table`` requires ``myst-parser``. The extension registers the MyST
parser integration, but ``myst-parser`` must be installed in the build
environment.
...
The leading apostrophe is removed from the rendered value. The resulting text
is then rendered as ordinary MyST Markdown cell content.
...
``mcsv-table`` supports the same interactive features as the other table
directives:
...
``mcsv-table`` supports these options:

``:file:``
   Path to an external UTF-8 ``.mcsv`` file. Do not combine it with inline
   content.
